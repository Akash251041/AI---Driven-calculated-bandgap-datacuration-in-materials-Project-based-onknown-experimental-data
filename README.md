...
│   ├── predict_general.ipynb   # 🔬 Band gap predictor — General model
│   └── predict_ternary.ipynb   # 🔬 Band gap predictor — Ternary model
│
└── README.md
```
...
| 5 | **Leave-One-Out (LOO)** | Per-sample generalization — strictest test of memorization vs. learning (General model only; 1,010 iterations) |
...
### Individual Sample Generalization — Leave-One-Out (LOO)

LOO validation trains on all 1,009 remaining samples and predicts the single held-out compound, repeated across all 1,010 entries of the General dataset. This is the strictest possible per-sample test of model memorization vs. genuine learning.

| Arity | Model | Iterations | R² | MAE (eV) | RMSE (eV) |
|-------|-------|-----------|-----|----------|-----------|
| **General** | **ExtraTrees** (n=300) | **1,010** | **0.8100** | **0.4631** | **0.7462** |

The LOO R² of **0.8100** actually **exceeds** the 10-Fold CV score (0.8001), and the MAE of **0.4631 eV** is lower than both 10-Fold CV (0.4825 eV) and Monte Carlo CV (0.4846 eV). This counter-intuitive result confirms there is no overfitting: the model trained on 1,009 samples predicts each individual held-out compound with equal or better accuracy than when trained on 80% of the data, demonstrating that the learned periodic-table heuristics are robust and fully transferable at the single-compound level.
...
Beyond benchmarking and validation, this framework includes two **interactive prediction notebooks** that allow anyone to predict the experimental band gap of any inorganic compound — including formulas completely outside the training dataset.

### Why only General and Ternary?

After thorough benchmarking and validation across all five compositional subsets, two models were selected for the prediction tool:

| Model | Reason for Selection |
|-------|---------------------|
| **General** | Trained on all 1,010 compounds across all arities. Achieves the best overall accuracy (R² = 0.8001, MAE = 0.4825 eV) and generalizes to any compound regardless of the number of constituent elements. |
| **Ternary** | Achieves the highest accuracy of any arity-specific model (R² = 0.8397, MAE = 0.3150 eV). For users working specifically with three-element compounds, this model provides superior precision. |

The remaining arity-specific models were not included because the Binary model has a higher MAE (0.6225 eV) than the General model, making the General model a strictly better choice for binary compounds too. The Quaternary model, while reasonable, does not offer a meaningful accuracy advantage over the General model (MAE = 0.3980 eV vs 0.4825 eV on a smaller subset), and the Unary model fails entirely due to insufficient data (21 samples).

### How to Use

Both prediction notebooks (`predict_general.ipynb` and `predict_ternary.ipynb`) follow the same simple 3-step workflow:

**Step 1 — Run once:** Installs packages and downloads all data from GitHub automatically.

**Step 2 — Run once:** Loads data, engineers all 276 features, and trains the ExtraTrees model on the full dataset (~1–2 minutes).

**Step 3 — Run as many times as you like:** Enter any chemical formula and its DFT-calculated band gap to get the predicted experimental band gap instantly.

```python
# ✏️ Edit only these two lines
INPUT_FORMULA  = "GaAs"   # any chemical formula
CALC_BAND_GAP  = 0.57     # DFT-calculated band gap in eV (from Materials Project)
```
...
==================================================
  Formula                  : GaAs
  Composition              : {'Ga': 0.5, 'As': 0.5}
  DFT-Calculated Band Gap  : 0.57 eV
  Predicted Experimental   : 1.4231 eV
==================================================
```

> **Note:** If the formula contains an element not in `Elements_DB.csv`, the notebook will still produce a best-estimate prediction with a warning. The model retains its full training knowledge in memory after Step 2, so no retraining is needed between predictions.
...
- **No memorization detected.** LOO validation on the General model (1,010 iterations) achieves R² = 0.8100 and MAE = 0.4631 eV — matching or exceeding 10-Fold CV performance — confirming the model generalizes at the individual sample level without overfitting.
