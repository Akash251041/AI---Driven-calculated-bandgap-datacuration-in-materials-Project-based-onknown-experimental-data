...
No local installation needed. All dependencies are installed automatically inside Google Colab. The notebooks include a setup cell that runs:

```python
!pip install xgboost catboost --quiet
```

All other libraries (`pandas`, `numpy`, `scikit-learn`, `matplotlib`) are pre-installed in Colab by default.

### Steps to Reproduce (Google Colab)

1. **Upload the data files to Google Drive**

   Create a folder in your Google Drive, e.g. `My Drive/BandGap_Project/data/`  
   Upload all 11 CSV files into that folder:
   - `general_mp.csv`, `general_exp.csv`
   - `binary_mp.csv`, `binary_exp.csv`
   - `ternary_mp.csv`, `ternary_exp.csv`
   - `quaternary_mp.csv`, `quaternary_exp.csv`
   - `unary_mp.csv`, `unary_exp.csv`
   - `Elements_DB.csv`

2. **Open a notebook in Google Colab**

   Go to [colab.research.google.com](https://colab.research.google.com) → **File → Upload notebook** → select e.g. `general.ipynb`

   Or open directly from GitHub:  
   **File → Open notebook → GitHub tab** → paste the repo URL → select a notebook

3. **Set your data folder path**

   At the top of each notebook, update `DATA_DIR` to match your Google Drive folder:
   ```python
   DATA_DIR = '/content/drive/MyDrive/BandGap_Project/data'
   ```

4. **Run all cells** (`Runtime → Run all`)

   Each notebook executes the full pipeline:
   - Mounts Google Drive and loads CSV files
