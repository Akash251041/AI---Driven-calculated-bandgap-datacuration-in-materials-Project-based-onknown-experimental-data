...
1. **Clone the repository**
   ```bash
   git clone https://github.com/Akash251041/AI---Driven-calculated-bandgap-datacuration-in-materials-Project-based-onknown-experimental-data.git
   cd AI---Driven-calculated-bandgap-datacuration-in-materials-Project-based-onknown-experimental-data
   ```

2. **Ensure all data files are in the `data/` directory** (10 CSV band gap files + `Elements_DB.csv`)

3. **Launch Jupyter and open a notebook**
   ```bash
   jupyter notebook notebooks/general.ipynb
   ```
   Or for a specific arity:
   ```bash
   jupyter notebook notebooks/binary.ipynb
   ```

4. **Run all cells.** Each notebook executes the full pipeline:
   - Loads and merges `_mp` + `_exp` CSV files
